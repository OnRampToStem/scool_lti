...
def init_platforms(data: SeedData) -> None:
...
def init_auth_users(data: SeedData) -> None:
    try:
        with SessionLocal.begin() as session:
...
def init_auth_json_web_keys(_: SeedData) -> None:
    web_keys = store.json_web_keys()
    if web_keys:
...
def init_db(data: SeedData) -> None:
