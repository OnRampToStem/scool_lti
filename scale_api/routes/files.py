...
def access_check(request: Request) -> None:
    ...
    auth_roles = {
        scope.lower() for scope in auth_user.scopes if scope.startswith("role")
    }
...
async def get_file(file_id: str):
    ...
    try:
        ...
        return Response(
            content=result.data, headers=headers, media_type=result.content_type
        )
...
async def put_file(file_id: str, file: UploadFile = File(...)):
    logger.debug(
        "files.put_file(%s)::%s (%s)", file_id, file.filename, file.content_type
    )
    try:
